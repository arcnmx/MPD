...
YtdlParseContext::YtdlParseContext() : depth(0), state(0), builder(std::make_unique<TagBuilder>()) { }
...
int YtdlParseContext::ytdl_yajl_string(void *ctx, const unsigned char *value, size_t len) {
	...
	switch (context->state) {
		...
		case YTDL_YAJL_STATE_UPLOADER: context->builder->AddItem(TAG_ARTIST, str.c_str()); break;
		case YTDL_YAJL_STATE_CREATOR: context->builder->AddItem(TAG_ALBUM_ARTIST, str.c_str()); break;
